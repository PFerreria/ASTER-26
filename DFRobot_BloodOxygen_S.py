...
import serial
import time
import smbus2 as smbus  # FIX: changed from 'smbus' to 'smbus2' for Python 3 / Raspberry Pi OS compatibility
import os
import math
...
class DFRobot_BloodOxygen_S(DFRobot_RTU):
  ...
  def begin(self):
    ...
    # FIX: read_reg can return an int (-1) on error, or a non-list type on newer smbus/smbus2.
    # Ensure rbuf is a subscriptable list before indexing.
    if not isinstance(rbuf, (list, bytearray)):
      return False
    if len(rbuf) >= 2 and (rbuf[0] & 0xff << 8 | rbuf[1]) == DEV_ADDRESS:
      return True
    else:
      return False
